...
final class WatchSessionStore {
    ...
    private func savePendingSessions() {
        ...
        if let lastError {
            Log.storage.error("All save attempts failed: \(lastError.localizedDescription)")
            saveToBackupLocation()
        }
    }

    /// Attempt to save to a backup location when primary save fails
    private func saveToBackupLocation() {
        guard let backupURL = sessionsFileURL?.deletingLastPathComponent()
            .appendingPathComponent("sessions_backup.json") else {
            return
        }
        ...
        do {
            ...
            Log.storage.info("Saved \(self.pendingSessions.count) sessions to backup location")
        } catch {
            Log.storage.error("Backup save also failed: \(error.localizedDescription)")
            // At this point we've done everything we can - sessions remain in memory
            // and will be saved on next successful attempt
        }
    }
    ...
    private func loadPendingSessions() {
        ...
        if fileManager.fileExists(atPath: url.path) {
            do {
                ...
                Log.storage.error("Failed to load sessions from primary: \(error.localizedDescription)")
            }
        }

        // Try backup location if primary failed or doesn't exist
        let backupURL = url.deletingLastPathComponent().appendingPathComponent("sessions_backup.json")
        if fileManager.fileExists(atPath: backupURL.path) {
            do {
                let data = try Data(contentsOf: backupURL)
                pendingSessions = try JSONDecoder().decode([WatchSession].self, from: data)
                Log.storage.info("Recovered \(self.pendingSessions.count) sessions from backup")

                // Move backup to primary location
                savePendingSessions()
                try? fileManager.removeItem(at: backupURL)
                return
            } catch {
                Log.storage.error("Failed to load sessions from backup: \(error.localizedDescription)")
            }
        }
