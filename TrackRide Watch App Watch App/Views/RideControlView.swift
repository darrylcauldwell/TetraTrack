...
struct RideControlView: View {
    ...
    @State private var lastHeartRateZone: Int = 0
    ...
    var body: some View {
        ...
        .onChange(of: connectivityService.distance) { _, newDistance in
            guard connectivityService.isRiding else { return }
        ...
        // HR zone change haptic
        .onChange(of: connectivityService.heartRateZone) { oldZone, newZone in
            guard connectivityService.isRiding, oldZone != newZone, newZone > 0 else { return }
            lastHeartRateZone = newZone
            HapticManager.shared.playZoneChangeHaptic(zone: newZone)
        }
    }
    ...
    private var controlButtons: some View {
        if connectivityService.isRiding {
            ...
            VStack(spacing: 8) {
                ...
                // Voice note button - only when paused
                if isPaused {
    ...
    private func toggleVoiceNote() {
        voiceService.onTranscriptionComplete = { text in
            // Send note to iPhone via connectivity service
