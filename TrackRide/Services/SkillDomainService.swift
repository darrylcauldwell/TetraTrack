...
final class SkillDomainService {
    ...
    func computeScores(from ride: Ride) -> [SkillDomainScore] {
        ...
        // STABILITY: riderStability + straightness + engagement
        let stabilityValue = computeRidingStability(ride)
        if stabilityValue > 0 {
            scores.append(SkillDomainScore(
                ...
                contributingMetrics: [
                    "riderStability": ride.averageRiderStability,
                    "straightness": ride.averageStraightness,
        ...
        // ENDURANCE: duration + trainingLoad
        ...
        if enduranceValue > 0 {
            scores.append(SkillDomainScore(
                ...
                contributingMetrics: [
                    ...
                    "trainingLoad": ride.totalTrainingLoad
                ]
            ))
        }
    ...
    private func computeRidingStability(_ ride: Ride) -> Double {
        // Weighted combination: 40% riderStability, 40% straightness, 20% engagement
        let riderStability = ride.averageRiderStability
        let straightness = ride.averageStraightness
        let engagement = ride.averageEngagement

        guard riderStability > 0 || straightness > 0 || engagement > 0 else { return 0 }

        return riderStability * 0.4 + straightness * 0.4 + engagement * 0.2
    }
    ...
    private func computeRidingBalance(_ ride: Ride) -> Double {
        ...
        if ride.leftTurns + ride.rightTurns > 0 {
    ...
    private func computeRidingEndurance(_ ride: Ride) -> Double {
        // Longer rides with training load = higher endurance
        let durationScore = min(100, ride.totalDuration / 3600 * 100)  // 1 hour = 100
        let loadScore = min(100, ride.totalTrainingLoad / 10 * 100)  // 10 units = 100

        guard durationScore > 0 || loadScore > 0 else { return 0 }

        if loadScore > 0 {
            return durationScore * 0.5 + loadScore * 0.5
        }
