...
extension InsightsCoordinator {
    ...
    private func calculateStats(for segments: [GaitSegment], in range: ClosedRange<Date>) -> RangeStats {
        ...
        guard !filtered.isEmpty else {
            return RangeStats(averageRhythm: 0, averageEngagement: 0, averageStraightness: 0, segmentCount: 0)
        }

        let rhythmSum = filtered.reduce(0) { $0 + $1.rhythmScore }
        let engagementSum = filtered.reduce(0) { $0 + $1.engagement }

        return RangeStats(
            averageRhythm: rhythmSum / Double(filtered.count),
            averageEngagement: engagementSum / Double(filtered.count),
            averageStraightness: 0,  // Would need location data
