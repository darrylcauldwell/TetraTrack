...
actor LiveTrackingService {
    ...
    /// Must be recreated each time because NWPathMonitor cannot be restarted after cancel().
    ...
    func configure(userID: String, zoneID: CKRecordZone.ID) {
        ...
        self.zoneID = zoneID
        // Network monitoring is deferred until startSharingLocation() to avoid
        // keeping NWPathMonitor running when the app is idle
    }

    /// Start monitoring network connectivity.
    /// Creates a fresh NWPathMonitor each time because cancel() is terminal.
    ...
    func startSharingLocation(riderName: String, activityType: String = "riding") async throws -> LiveTrackingSession {
        ...
        // Start network monitoring only while actively sharing
    ...
    func stopSharingLocation() async throws {
        ...
        // Stop network monitoring — no longer needed when not sharing
        stopNetworkMonitoring()
    ...
    private func updateRecord(_ record: CKRecord, from session: LiveTrackingSession) {
        ...
        record["activityType"] = session.activityType
    ...
    private func sessionFromRecord(_ record: CKRecord) -> LiveTrackingSession? {
        ...
        session.activityType = record["activityType"] as? String ?? "riding"
