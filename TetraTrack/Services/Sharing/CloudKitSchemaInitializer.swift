...
actor CloudKitSchemaInitializer {
    ...
    private func ensureRecordTypeExists(_ definition: RecordTypeDefinition) async -> Bool {
        ...
        // Note: Record names cannot start with __ (reserved for CloudKit system use)
        ...
        do {
            ...
            // Handle specific errors with detailed diagnostics
            var detailedError = "[\(definition.name)] CKError code: \(error.code.rawValue) (\(error.code))"

            // Extract partial errors which contain field-level details
            if let partialErrors = error.userInfo[CKPartialErrorsByItemIDKey] as? [AnyHashable: Error] {
                for (key, partialError) in partialErrors {
                    detailedError += " | Partial[\(key)]: \(partialError.localizedDescription)"
                }
            }

            // Check for underlying error with more details
            if let underlying = error.userInfo[NSUnderlyingErrorKey] as? Error {
                detailedError += " | Underlying: \(underlying.localizedDescription)"
            }

            // Include any error description from CloudKit
            if let errorDescription = error.userInfo["CKErrorDescription"] as? String {
                detailedError += " | CKDesc: \(errorDescription)"
            }

            // Add the full userInfo keys for debugging
            let userInfoKeys = error.userInfo.keys.map { "\($0)" }.joined(separator: ", ")
            detailedError += " | UserInfo keys: [\(userInfoKeys)]"

            Log.family.error("CloudKit schema error: \(detailedError)")

            switch error.code {
            case .serverRejectedRequest:
                // Schema might already exist, which is fine
                Log.family.debug("Record type \(definition.name) may already exist")
                initializationErrors.append("\(definition.name): Server rejected - \(error.localizedDescription)")
                return false
            case .invalidArguments:
                // Field type mismatch - schema exists but is different
                initializationErrors.append("Schema mismatch for \(definition.name): \(detailedError)")
                return false
            default:
                initializationErrors.append("Failed \(definition.name): \(detailedError)")
                return false
            }
        } catch {
