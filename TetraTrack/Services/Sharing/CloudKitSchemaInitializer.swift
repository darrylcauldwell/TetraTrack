...
enum CloudKitSchema {
    ...
    static nonisolated let liveTrackingSession = RecordTypeDefinition(
    ...
    static nonisolated let safetyAlert = RecordTypeDefinition(
        name: "SafetyAlert",
        fields: [
            .string("riderName"),
            .string("riderID"),
    ...
    static nonisolated let shareConnection = RecordTypeDefinition(
    ...
    static nonisolated var allRecordTypes: [RecordTypeDefinition] {
...
actor CloudKitSchemaInitializer {
    ...
    private func ensureRecordTypeExists(_ definition: RecordTypeDefinition) async -> Bool {
        ...
        // First check if the record type already exists via a query
        // This avoids template record conflicts with existing schemas from old versions
        if await validateRecordType(definition, zoneID: zoneID) {
            Log.family.debug("Record type \(definition.name) already exists, skipping template creation")
            return false // exists but not newly created
        }

        // Record type doesn't exist yet — create a template record to initialize the schema
        // CloudKit auto-creates schema from records saved in Development mode
        let recordID = CKRecord.ID(
            recordName: "schema-template-\(definition.name)-\(UUID().uuidString.prefix(8))",
            zoneID: zoneID
        )
        ...
        do {
            ...
            Log.family.debug("Template save for \(definition.name) got CKError \(error.code.rawValue)")

            switch error.code {
            case .serverRejectedRequest, .invalidArguments:
                // Schema likely already exists (possibly with different fields from old versions)
                // This is not a blocking error — the record type exists in CloudKit
                Log.family.debug("Record type \(definition.name) likely exists with existing schema")
                return false
            default:
                let desc = error.localizedDescription
                Log.family.error("Failed to create schema for \(definition.name): \(desc)")
                initializationErrors.append("Failed \(definition.name): CKError \(error.code.rawValue) - \(desc)")
                return false
            }
        } catch {
    ...
    private func validateRecordType(_ definition: RecordTypeDefinition, zoneID: CKRecordZone.ID) async -> Bool {
        ...
        do {
            ...
            let (_, _) = try await container.privateCloudDatabase.records(
                matching: query,
                inZoneWith: zoneID,
                resultsLimit: 1
            )
