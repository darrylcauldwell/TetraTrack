...
final class UnifiedSharingCoordinator {
    ...
    func generateShareLink(for relationship: SharingRelationship) async -> URL? {
        ...
        do {
            ...
            switch error {
            case .notSignedIn:
                errorMessage = "iCloud authentication failed. Please sign out and back into iCloud in Settings, then try again."
            case .zoneNotAvailable:
                errorMessage = "Could not access iCloud storage. Please check iCloud Drive is enabled in Settings > [Your Name] > iCloud."
            case .shareCreationFailed(let underlying):
                Log.family.error("Share creation underlying error: \(underlying)")
                let ckInfo: String
                if let ckError = underlying as? CKError {
                    ckInfo = " (CK\(ckError.code.rawValue))"
                } else {
                    ckInfo = ""
                }
                errorMessage = "Failed to create share link\(ckInfo): \(underlying.localizedDescription)\n\nIf this persists, try: Settings > [Your Name] > iCloud > ensure iCloud Drive is ON."
            case .shareNotFound:
                errorMessage = "Share not found. Please try again."
            case .recordNotFound:
                errorMessage = "Record not found in iCloud."
            case .invalidShareURL:
                errorMessage = "Invalid share URL generated. Please check your iCloud settings."
    ...
    private func saveLinkedRiders() {
        guard repository?.modelContext != nil else { return }
    ...
    /// Reset all sharing state: clear service caches, linked riders, and rate limits.
    ...
    func resetAllSharingState() async {
        ...
        // 3. Delete LinkedRiderRecord entries from SwiftData
        if let context = repository?.modelContext {
            do {
                let descriptor = FetchDescriptor<LinkedRiderRecord>()
                let records = try context.fetch(descriptor)
                for record in records {
                ...
                try context.save()
                Log.family.info("Deleted \(records.count) LinkedRiderRecord(s) from SwiftData")
            } catch {
                Log.family.error("Failed to delete LinkedRiderRecords: \(error)")
            }
        }

        // 4. Clear linked riders from UserDefaults (legacy)
        UserDefaults.standard.removeObject(forKey: linkedRidersUserDefaultsKey)

        // 5. Clear in-memory state
        linkedRiders.removeAll()
        sharedWithMe.removeAll()
        mySession = nil
        errorMessage = nil

        // 6. Clear cached connection info from all relationships
        if let repository = repository {
            do {
                ...
                Log.family.info("Cleared connection info from \(relationships.count) relationship(s)")
            } catch {
                Log.family.error("Failed to clear relationship cache: \(error)")
            }
        }

        Log.family.info("UnifiedSharingCoordinator: All sharing state reset")
    }
