...
final class FallDetectionManager: FallDetecting {
    ...
    private var countdownTimerSource: DispatchSourceTimer?
    private let countdownQueue = DispatchQueue(label: "dev.dreamfold.tetratrack.fallCountdown", qos: .userInteractive)
    ...
    private func startCountdown() {
        ...
        source.resume()
        countdownTimerSource = source
    }

    private func stopCountdown() {
        countdownTimerSource?.cancel()
        countdownTimerSource = nil
    }
    ...
    private func sendEmergencyNotifications() async {
        ...
        let descriptor = FetchDescriptor<SharingRelationship>(
            predicate: #Predicate { $0.receiveFallAlerts == true && $0.phoneNumber != nil }
        )
        ...
        do {
            ...
            if contacts.isEmpty {
                Log.safety.warning("No emergency contacts with phone numbers configured")
            }

            // Send notifications via NotificationManager (also creates CloudKit alert)
            ...
            Log.safety.info("Emergency notifications sent to \(contacts.count) contacts with phone numbers")
