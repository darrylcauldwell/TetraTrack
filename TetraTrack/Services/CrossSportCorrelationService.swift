...
final class CrossSportCorrelationService {
    ...
    static let knownTransfers: [(source: TrainingDiscipline, target: TrainingDiscipline, mechanism: String, domain: SkillDomain)] = [
        ...
        // Walking → Running transfers (walking is a RunningSessionType, uses .running discipline)
        (.running, .running, "Walking symmetry predicts running gait quality", .symmetry),
        (.running, .running, "Walking cadence consistency transfers to running rhythm", .rhythm),
        (.running, .running, "Walking stability correlates with running form steadiness", .stability)
    ]
