...
final class LocationManager: NSObject {
    ...
    // Multi-subscriber location callbacks
    private var locationSubscribers: [String: (CLLocation) -> Void] = [:]

    /// Subscribe to location updates with a unique identifier.
    /// Multiple subscribers can coexist — each receives every location update.
    func subscribe(id: String, handler: @escaping (CLLocation) -> Void) {
        locationSubscribers[id] = handler
    }

    /// Remove a location subscriber by its identifier.
    func unsubscribe(id: String) {
        locationSubscribers.removeValue(forKey: id)
    }
    ...
    func startTracking() async {
        ...
        // Enable background location updates for any authorized state
        // App has UIBackgroundModes: location in Info.plist
        // CLBackgroundActivitySession works with When In Use auth (iOS 17+)
                ...
                for try await update in updates {
                    ...
                    if let location = update.location {
                        await MainActor.run {
                            ...
                            if self.locationSubscribers.isEmpty {
                                Log.location.warning("Location update received but no subscribers registered")
                            } else {
                                for (_, handler) in self.locationSubscribers {
                                    handler(location)
                                }
                            }
                        }
