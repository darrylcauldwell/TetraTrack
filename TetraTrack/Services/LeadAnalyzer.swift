...
final class LeadAnalyzer: Resettable {
    ...
    /// Phone mount position affects signal quality and confidence weighting
    private(set) var phoneMountPosition: PhoneMountPosition = .jodhpurThigh
    ...
    /// Rolling buffer of lateral acceleration samples (EMA-filtered)
    private var lateralBuffer: [Double] = []

    /// Rolling buffer of yaw rate samples
    private var yawBuffer: [Double] = []

    /// Last filtered lateral value for EMA smoothing
    private var lastFilteredLateral: Double?
    ...
    func processMotionSample(_ sample: MotionSample, currentGait: GaitType) {
        ...
        // Add to buffers with EMA filtering based on phone placement
        let alpha = phoneMountPosition.filterAlpha
        let filteredLateral: Double
        if let prev = lastFilteredLateral {
            filteredLateral = alpha * sample.lateralAcceleration + (1 - alpha) * prev
        } else {
            filteredLateral = sample.lateralAcceleration
        }
        lastFilteredLateral = filteredLateral
        lateralBuffer.append(filteredLateral)
    ...
    /// Configure phone mount position for confidence weighting
    func configure(phoneMountPosition: PhoneMountPosition) {
        self.phoneMountPosition = phoneMountPosition
    }
    ...
    private func analyzeLeadWithPhase() {
        ...
        // Adjust confidence based on phone placement
        // Thigh pocket gives stronger lateral signal for lead detection
        // Chest pocket has weaker lateral component and more torso dampening
        let placementMultiplier: Double = switch phoneMountPosition {
        case .jodhpurThigh: 1.0
        case .jacketChest: 0.75
        }
        finalConfidence *= placementMultiplier
