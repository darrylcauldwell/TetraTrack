...
struct GaitDiagnosticEntry: Codable {
    let timestamp: Date
    ...
    let xyCoherence: Double
    let zYawCoherence: Double
    let normalizedVerticalRMS: Double
    let yawRateRMS: Double
    let gpsSpeed: Double
...
final class GaitHMM {
    ...
    var stateProbabilitiesDict: [String: Double] {
    ...
    /// Only adjusts if enough rides have been recorded (minimum 3)
    /// Blend factor increases with more rides but caps at 0.5 to never fully override breed priors
    func applyLearnedParameters(_ learned: LearnedGaitParameters) {
        guard learned.rideCount >= 3 else { return }

        // Blend factor: how much to trust learned data vs breed defaults
    ...
    /// Blend a single emission mean toward a learned target value
    ...
    private static func defaultTransitionMatrix() -> [[Double]] {
        ...
        // Self-transition probability: 0.85 provides good balance between
        // stability (removing flicker) and responsiveness to actual transitions.
        // At 6 Hz update rate, this allows transitions within ~1-1.5 seconds of sustained change.
        let selfProb = 0.85
        let transProb = 0.15
