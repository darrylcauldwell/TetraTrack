...
private extension IntelligenceService {
    ...
    func buildRunningAnalysisPrompt(_ sessions: [RunningSession]) -> String {
        let recentSessions = sessions.filter { $0.sessionType != .walking }.prefix(10)
        ...
        // Aggregate Watch sensor metrics
        let sessionsWithOsc = recentSessions.filter { $0.averageVerticalOscillation > 0 }
        let avgOscillation = sessionsWithOsc.isEmpty ? 0 : sessionsWithOsc.reduce(0) { $0 + $1.averageVerticalOscillation } / Double(sessionsWithOsc.count)
        let sessionsWithGCT = recentSessions.filter { $0.averageGroundContactTime > 0 }
        let avgGCT = sessionsWithGCT.isEmpty ? 0 : sessionsWithGCT.reduce(0) { $0 + $1.averageGroundContactTime } / Double(sessionsWithGCT.count)
        let sessionsWithBreathing = recentSessions.filter { $0.averageBreathingRate > 0 }
        let avgBreathing = sessionsWithBreathing.isEmpty ? 0 : sessionsWithBreathing.reduce(0) { $0 + $1.averageBreathingRate } / Double(sessionsWithBreathing.count)
        ...
        let sessionsWithFatigue = recentSessions.filter { $0.endFatigueScore > 0 }
        let avgFatigue = sessionsWithFatigue.isEmpty ? 0 : sessionsWithFatigue.reduce(0) { $0 + $1.endFatigueScore } / Double(sessionsWithFatigue.count)
        let sessionsWithPosture = recentSessions.filter { $0.postureStability > 0 }
        let avgPosture = sessionsWithPosture.isEmpty ? 0 : sessionsWithPosture.reduce(0) { $0 + $1.postureStability } / Double(sessionsWithPosture.count)

        var sensorMetrics = ""
        if avgOscillation > 0 { sensorMetrics += "\nAverage vertical oscillation: \(String(format: "%.1f", avgOscillation)) cm" }
        if avgGCT > 0 { sensorMetrics += "\nAverage ground contact time: \(String(format: "%.0f", avgGCT)) ms" }
        ...
        if avgFatigue > 0 { sensorMetrics += "\nAverage fatigue score: \(String(format: "%.0f", avgFatigue))/100" }
        if avgPosture > 0 { sensorMetrics += "\nAverage posture stability: \(String(format: "%.0f", avgPosture))/100" }

        // Walking session context
        let walkingSessions = sessions.filter { $0.sessionType == .walking }
        var walkingContext = ""
        if !walkingSessions.isEmpty {
            let avgSymmetry = walkingSessions.reduce(0) { $0 + $1.walkingSymmetryScore } / Double(walkingSessions.count)
            let avgRhythm = walkingSessions.reduce(0) { $0 + $1.walkingRhythmScore } / Double(walkingSessions.count)
            let avgStability = walkingSessions.reduce(0) { $0 + $1.walkingStabilityScore } / Double(walkingSessions.count)
            walkingContext = """

            WALKING SESSIONS (\(walkingSessions.count) total):
            - Average symmetry score: \(String(format: "%.0f", avgSymmetry))/100
            - Average rhythm score: \(String(format: "%.0f", avgRhythm))/100
            - Average stability score: \(String(format: "%.0f", avgStability))/100
            Walking biomechanics are a strong predictor of running form quality. Consider how walking metrics inform running improvement.
            """
        }

        // Training program context
        var programContext = ""
        let programSessions = sessions.filter { $0.programSessionId != nil }
        if !programSessions.isEmpty {
            programContext = """

            TRAINING PROGRAM:
            The athlete is following a structured training program (\(programSessions.count) program sessions recorded).
            Consider program progression and adherence in your analysis.
            """
        }
