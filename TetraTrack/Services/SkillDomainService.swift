...
final class SkillDomainService {
    ...
    func computeScores(from session: RunningSession, score: RunningScore?) -> [SkillDomainScore] {
        // Walking sessions use dedicated biomechanics scoring
        if session.sessionType == .walking {
            return computeWalkingScores(from: session)
        }
    ...
    // MARK: - Walking Score Computation

    /// Compute domain scores from a walking session using biomechanics
    private func computeWalkingScores(from session: RunningSession) -> [SkillDomainScore] {
        var scores: [SkillDomainScore] = []

        // SYMMETRY: from walking biomechanics (HealthKit asymmetry)
        if session.walkingSymmetryScore > 0 {
            scores.append(SkillDomainScore(
                domain: .symmetry,
                score: session.walkingSymmetryScore,
                confidence: session.healthKitAsymmetry != nil ? 0.9 : 0.5,
                discipline: .running,
                sourceSessionId: session.id,
                contributingMetrics: [
                    "walkingSymmetryScore": session.walkingSymmetryScore,
                    "healthKitAsymmetry": session.healthKitAsymmetry ?? 0
                ]
            ))
        }

        // RHYTHM: from cadence consistency
        if session.walkingRhythmScore > 0 {
            scores.append(SkillDomainScore(
                domain: .rhythm,
                score: session.walkingRhythmScore,
                confidence: 0.7,
                discipline: .running,
                sourceSessionId: session.id,
                contributingMetrics: [
                    "walkingRhythmScore": session.walkingRhythmScore,
                    "cadenceConsistency": session.walkingCadenceConsistency,
                    "averageCadence": Double(session.averageCadence)
                ]
            ))
        }

        // STABILITY: from speed variability
        if session.walkingStabilityScore > 0 {
            scores.append(SkillDomainScore(
                domain: .stability,
                score: session.walkingStabilityScore,
                confidence: 0.7,
                discipline: .running,
                sourceSessionId: session.id,
                contributingMetrics: [
                    "walkingStabilityScore": session.walkingStabilityScore
                ]
            ))
        }

        // ENDURANCE: walking duration
        let endurance = min(100, session.totalDuration / 36) // 60 min = 100
        if endurance > 0 {
            scores.append(SkillDomainScore(
                domain: .endurance,
                score: endurance,
