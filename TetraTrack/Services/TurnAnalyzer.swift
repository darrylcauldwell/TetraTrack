...
final class TurnAnalyzer: Resettable {
    ...
    private func updatePhysicalTurnModel(angleDiff: Double, timestamp: Date) {
        ...
        if absAngle < PhysicalTurnModel.noiseThreshold {
            ...
            if let turn = physicalTurnInProgress {
    ...
    func generateOverSegmentationReport() -> TurnOverSegmentationReport {
        ...
        // Over-count factor (using physical turn event counts)
        let totalEvents = completedPhysicalTurns.reduce(0) { $0 + $1.detectedEventCount }
        let overCountFactor = physicalTurnCount > 0 ? Double(totalEvents) / Double(physicalTurnCount) : 0
