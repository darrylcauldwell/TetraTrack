...
final class ShootingHistoryService {
    ...
    // MARK: - Context-Aware Analytics

    /// Get patterns filtered by session type (context)
    func patterns(for sessionTypes: Set<ShootingSessionType>, from allPatterns: [StoredTargetPattern]) -> [StoredTargetPattern] {
        allPatterns.filter { sessionTypes.contains($0.sessionType) }
    }

    /// Get patterns by pressure level
    func patterns(forPressureLevel level: Int, from allPatterns: [StoredTargetPattern]) -> [StoredTargetPattern] {
        allPatterns.filter { $0.sessionType.pressureLevel == level }
    }

    /// Calculate average spread for a specific pressure level
    func averageSpread(forPressureLevel level: Int, patterns: [StoredTargetPattern]) -> Double? {
        let filtered = patterns.filter { $0.sessionType.pressureLevel == level }
        guard !filtered.isEmpty else { return nil }
        return filtered.map { $0.clusterRadius }.reduce(0, +) / Double(filtered.count)
    }

    /// Calculate average spread for specific session types
    func averageSpread(for sessionTypes: Set<ShootingSessionType>, patterns: [StoredTargetPattern]) -> Double? {
        let filtered = patterns.filter { sessionTypes.contains($0.sessionType) }
        guard !filtered.isEmpty else { return nil }
        return filtered.map { $0.clusterRadius }.reduce(0, +) / Double(filtered.count)
    }

    /// Compare bias patterns across different contexts
    func biasComparison(patterns: [StoredTargetPattern]) -> BiasComparisonResult {
        var resultsByPressure: [Int: BiasMetrics] = [:]

        for level in 1...3 {
            let filtered = patterns.filter { $0.sessionType.pressureLevel == level }
            guard !filtered.isEmpty else { continue }

            let avgMpiX = filtered.map { $0.clusterMpiX }.reduce(0, +) / Double(filtered.count)
            let avgMpiY = filtered.map { $0.clusterMpiY }.reduce(0, +) / Double(filtered.count)
            let avgSpread = filtered.map { $0.clusterRadius }.reduce(0, +) / Double(filtered.count)
            let offset = sqrt(avgMpiX * avgMpiX + avgMpiY * avgMpiY)

            resultsByPressure[level] = BiasMetrics(
                averageMpiX: avgMpiX,
                averageMpiY: avgMpiY,
        ...
        return BiasComparisonResult(byPressureLevel: resultsByPressure)
    }

    /// Comprehensive pressure analysis comparing low vs high pressure performance
    func pressureAnalysis(patterns: [StoredTargetPattern]) -> PressureAnalysisResult {
        let lowPressure = patterns.filter { $0.sessionType.pressureLevel == 1 }
        let highPressure = patterns.filter { $0.sessionType.pressureLevel >= 2 }

        // Calculate metrics for each group
        let lowMetrics = computeMetrics(patterns: lowPressure)
        ...
        // Determine impact
        let impact: PressureImpact
        if let change = spreadChange {
            if change > 20 {
                impact = .significant
            } else if change > 10 {
                impact = .moderate
                ...
                impact = .positive  // Performs better under pressure
            }
        } else {
        ...
        return PressureAnalysisResult(
            lowPressureMetrics: lowMetrics,
            highPressureMetrics: highMetrics,
            spreadChangePercent: spreadChange,
            impact: impact,
            lowPressureCount: lowPressure.count,
            highPressureCount: highPressure.count
        )
    }
}

// MARK: - Context Analytics Models

/// Metrics for bias comparison
struct BiasMetrics {
    let averageMpiX: Double
    let averageMpiY: Double
    let averageSpread: Double
    let offset: Double
    let sampleCount: Int

    var biasDirection: String {
        let threshold = 0.03
...
/// Result of bias comparison across pressure levels
struct BiasComparisonResult {
    let byPressureLevel: [Int: BiasMetrics]

    var hasDataForComparison: Bool {
        byPressureLevel.count >= 2
    }

    /// Get description of bias shift under pressure
    var biasShiftDescription: String? {
        guard let low = byPressureLevel[1], let high = byPressureLevel[3] ?? byPressureLevel[2] else {
            return nil
        }

        let xShift = high.averageMpiX - low.averageMpiX
        let yShift = high.averageMpiY - low.averageMpiY

        var shifts: [String] = []
        if abs(xShift) > 0.05 {
            shifts.append(xShift > 0 ? "right" : "left")
        }
        if abs(yShift) > 0.05 {
            shifts.append(yShift > 0 ? "low" : "high")
        }

        if shifts.isEmpty {
            return "Your point of impact stays consistent under pressure."
        } else {
            return "Under pressure, your shots tend to shift \(shifts.joined(separator: " and "))."
        }
    }
}

/// Impact of pressure on performance
enum PressureImpact: String {
    case significant = "Significant impact"
    case moderate = "Moderate impact"
    case minimal = "Minimal impact"
    case positive = "Positive effect"
    case insufficientData = "Need more data"
...
/// Comprehensive pressure analysis result
struct PressureAnalysisResult {
    let lowPressureMetrics: AggregatedShootingMetrics
    let highPressureMetrics: AggregatedShootingMetrics
    let spreadChangePercent: Double?
    let impact: PressureImpact
    let lowPressureCount: Int
    let highPressureCount: Int

    var hasEnoughData: Bool {
        lowPressureCount >= 3 && highPressureCount >= 3
    }

    var summary: String {
        guard let change = spreadChangePercent else {
            return "Not enough data to compare pressure effects. Keep practicing across different contexts."
        }

        switch impact {
        case .significant:
            return "Your groups widen by \(Int(change))% under pressure. Mental preparation and breathing exercises may help."
        case .moderate:
            return "Pressure causes your groups to widen by \(Int(change))%. Simulated competition practice can help."
        case .minimal:
            return "Your consistency is excellent across pressure levels (\(Int(abs(change)))% variation)."
        case .positive:
            return "You actually shoot tighter groups under pressure (\(Int(abs(change)))% improvement)! You thrive with stakes."
        case .insufficientData:
            return "Complete more sessions across different contexts to see pressure effects."
        }
    }
}
