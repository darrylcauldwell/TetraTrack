...
actor EnhancedTargetScanner {
    ...
    func runAutoDetection(state: EnhancedScannerState) async {
        let stateSnapshot = await MainActor.run {
            (image: state.croppedImage?.cgImage, geometry: state.cropGeometry)
        }
        guard let image = stateSnapshot.image,
              let geometry = stateSnapshot.geometry else {
            await MainActor.run {
                state.phase = .manualCorrection
            }
            return
    ...
    func addManualShot(
        ...
        guard let geometry = await MainActor.run(body: { state.cropGeometry }) else { return }
    ...
    func confirmDetectedHole(
        ...
        let stateSnapshot = await MainActor.run {
            (geometry: state.cropGeometry, imageSize: state.croppedImage?.size)
        }
        guard let geometry = stateSnapshot.geometry,
              let imageSize = stateSnapshot.imageSize else { return }
    ...
    func acceptAllCandidates(
        ...
        let toAccept = await MainActor.run {
            state.detectedHoles.filter { $0.confidence >= minConfidence }
        }
    ...
    func finalizeScan(
        ...
        let stateSnapshot = await MainActor.run { () -> (shots: [ScanShot], geometry: TargetCropGeometry?, imageQuality: ImageQualityAssessment?, alignment: TargetAlignment?)? in
            ...
            return (state.confirmedShots, geometry, state.imageQuality, state.targetAlignment)
        }
        ...
        let validationResult = await MainActor.run {
            ShotValidator.validateShotCollection(
        ...
        let patternAnalysis = await MainActor.run {
            PatternAnalyzer.analyze(shots: snapshot.shots)
        }
        ...
        nonisolated(unsafe) let analysis = await MainActor.run {
    ...
    private func buildAcquisitionQuality(
        ...
        // Calculate perspective severity from alignment geometry
        let perspectiveSeverity = calculatePerspectiveSeverity(from: alignment)
        ...
        return AcquisitionQuality(
            ...
            perspectiveSeverity: perspectiveSeverity,
    ...
    /// Calculate perspective severity from target alignment geometry.
    /// Returns 0 (no perspective distortion) to 1 (severe distortion).
    private func calculatePerspectiveSeverity(from alignment: TargetAlignment?) -> Double {
        guard let alignment else { return 0.2 }  // Default for missing alignment

        // Factor 1: Aspect ratio deviation from expected physical ratio
        // Expected ratio for tetrathlon targets is ~0.77 (height/width)
        let expectedAspectRatio = 0.77
        let actualAspectRatio = alignment.confirmedSemiAxes.height / max(alignment.confirmedSemiAxes.width, 0.001)
        let aspectDeviation = abs(actualAspectRatio - expectedAspectRatio) / expectedAspectRatio

        // Factor 2: Rotation indicates camera angle
        // More than 5 degrees suggests significant perspective
        let rotationFactor = min(abs(alignment.rotationAdjustment) / 15.0, 1.0)

        // Factor 3: Center offset from expected center
        // Large offsets indicate keystone/trapezoid distortion
        let centerOffsetMagnitude = sqrt(
            alignment.centerOffset.x * alignment.centerOffset.x +
            alignment.centerOffset.y * alignment.centerOffset.y
        )
        let centerOffsetFactor = min(centerOffsetMagnitude / 0.1, 1.0)

        // Combined severity (weighted average)
        let severity = (aspectDeviation * 0.5) + (rotationFactor * 0.3) + (centerOffsetFactor * 0.2)
