...
struct DetectionCandidate: Identifiable {
    ...
    nonisolated var signalCount: Int { signals.count }

    /// Best raw score across all signals
    nonisolated var bestRawScore: Double {
        rawScores.values.max() ?? 0.0
    }

    /// Merge another candidate into this one
    nonisolated mutating func merge(with other: DetectionCandidate) {
...
struct CandidateFeatures {
    ...
    nonisolated static var empty: CandidateFeatures {
        CandidateFeatures(
            intensityDelta: 0,
            contrastRatio: 0,
            edgeClosure: 0,
...
struct HoleDetectionConfiguration: Sendable {
    // Size parameters
...
struct Blob {
    ...
    nonisolated var area: Int { pixels.count }

    nonisolated var centroid: CGPoint {
    ...
    nonisolated var perimeter: Double {
        var boundaryCount = 0
    ...
    nonisolated var compactness: Double {
        let p = perimeter
