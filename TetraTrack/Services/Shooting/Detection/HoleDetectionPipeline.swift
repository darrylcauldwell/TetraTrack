...
actor HoleDetectionPipeline {
    ...
    private func logDiagnostics(_ diagnostics: DetectionDiagnostics, regionAnalysis: TargetRegionAnalysis) {
        ...
        for cand in diagnostics.allCandidates.prefix(10) {
            ...
            _ = cand.signals.map { $0.rawValue.prefix(4) }.joined(separator: ",")
            let featStr: String
...
enum ImageProcessor {
    ...
    nonisolated static func toGrayscale(cgImage: CGImage) -> (grayscale: [[UInt8]], width: Int, height: Int)? {
    ...
    nonisolated static func sobelMagnitude(grayscale: [[UInt8]], width: Int, height: Int) -> [[UInt8]] {
...
extension DetectedHole {
    nonisolated init(position: CGPoint, score: Int, confidence: Double, radius: Double, needsReview: Bool = false, reviewReason: String? = nil) {
        self.id = UUID()
