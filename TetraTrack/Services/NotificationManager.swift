...
final class NotificationManager: NSObject {
    ...
    // CloudKit enabled - iCloud entitlement configured in TetraTrack.entitlements
    ...
    nonisolated func sendLocalStationaryWarning(riderName: String, duration: TimeInterval) {
    ...
    nonisolated func sendLocalUrgentAlert(riderName: String, duration: TimeInterval) {
        let content = UNMutableNotificationContent()
    ...
    private func handleSafetyAlertNotification(recordID: CKRecord.ID, notification: CKQueryNotification) async {
        ...
        do {
            ...
            // Fall back to showing a generic safety alert notification
            await showSafetyAlertNotification(
                title: "Safety Alert",
                message: "A safety alert was received from a family member.",
    ...
    func sendFallDetectionAlert(
        ...
        let alertMessage = """
⚠️ EMERGENCY ALERT from TetraTrack
...
This is an automated message from TetraTrack.
"""

        // Filter to contacts with phone numbers who have fall alerts enabled
        let reachableContacts = contacts.filter { $0.receiveFallAlerts && $0.smsURLWithBody(alertMessage) != nil }

        // Send SMS to reachable emergency contacts
        for contact in reachableContacts {
            await sendEmergencySMS(to: contact, message: alertMessage)
        }

        if reachableContacts.isEmpty {
            Log.notifications.warning("No emergency contacts with valid phone numbers — SMS alerts not sent")
        } else {
            Log.notifications.info("SMS alerts prepared for \(reachableContacts.count) contact(s)")
        }

        // Send a local notification with accurate messaging
        sendLocalFallAlert(riderName: riderName, contactsReached: reachableContacts.count)

        // Create CloudKit record for family members (works even without phone numbers)
        if let loc = location {
    ...
    private func sendEmergencySMS(to contact: SharingRelationship, message: String) async {
        // On iOS, we can't send SMS programmatically without user interaction
        // However, we can prepare the SMS URL and open it
        guard let smsURL = contact.smsURLWithBody(message) else {
            Log.notifications.warning("Skipping SMS for \(contact.name) — no valid phone number")
            return
        }
    ...
    func sendLocalFallAlert(riderName: String, contactsReached: Int = 0) {
        let content = UNMutableNotificationContent()
        content.title = "🚨 Fall Detected"
        if contactsReached > 0 {
            content.body = "Alerting \(contactsReached) emergency contact\(contactsReached == 1 ? "" : "s"). Help is on the way."
        } else {
            content.body = "No emergency contacts with phone numbers configured. Call for help manually."
        }
        content.sound = .defaultCritical
    ...
    nonisolated func clearNotificationsForRider(_ riderID: String) {
