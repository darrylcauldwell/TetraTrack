...
// MARK: - Running Coaching Level

/// Quick coaching verbosity presets for running sessions
enum RunningCoachingLevel: String, CaseIterable, Identifiable {
...
final class AudioCoachManager: AudioCoaching {
    ...
    // MARK: - Mute (session-scoped, not persisted)
    ...
    private(set) var sessionAnnouncementCount: Int = 0
    ...
    func resetSessionAnnouncementCount() {
        sessionAnnouncementCount = 0
    }

    func applyRunningCoachingLevel(_ level: RunningCoachingLevel) {
        runningCoachingLevel = level
        switch level {
        case .silent:
            announceRunningPace = false
            announceRunningLaps = false
...
extension AudioCoachManager {
    ...
    /// Default optimal cadence range for running (steps per minute)
    private static let defaultOptimalCadenceRange = 170...180

    /// Build optimal range from target cadence (±5 SPM), or use default
    private static func optimalRange(for target: Int) -> ClosedRange<Int> {
        guard target > 0 else { return defaultOptimalCadenceRange }
        return (target - 5)...(target + 5)
    }

    /// Announce cadence feedback if outside optimal range
    func processCadence(_ cadence: Int, target: Int = 0) {
        guard isEnabled else { return }
        ...
        let range = Self.optimalRange(for: target)

        // Only announce if significantly outside optimal range
        if cadence < range.lowerBound - 5 {
            Self.lastCadenceWarning = Date()
            if target > 0 {
                announce("Cadence \(cadence), target is \(target). Pick up your step rate")
            } else {
                announce("Cadence \(cadence). Try to increase your step rate")
            }
        } else if cadence > range.upperBound + 10 {
            Self.lastCadenceWarning = Date()
            if target > 0 {
                announce("Cadence \(cadence), target is \(target). Ease off your step rate")
            } else {
    ...
    func announceCadence(_ cadence: Int, target: Int = 0) {
        guard isEnabled else { return }

        let range = Self.optimalRange(for: target)

        var message = "Cadence \(cadence) steps per minute"
        if range.contains(cadence) {
            message += ". Good rhythm"
        } else if cadence < range.lowerBound {
...
// MARK: - Tetrathlon Practice Coaching

extension AudioCoachManager {
    /// Announce start of tetrathlon practice session with standard time and PB context
    func announceTetrathlonPracticeStart(
        pbTime: TimeInterval,
        ...
        let distanceFormatted = formatDistanceForSpeech(distance)
        let standardFormatted = formatDurationForSpeech(standardTime)

        var message = "Tetrathlon practice. \(distanceFormatted). 1000-point target: \(standardFormatted)."

        if pbTime > 0 {
            let pbFormatted = formatDurationForSpeech(pbTime)
            let pbPoints = 1000.0 - ((pbTime - standardTime) * 3.0)
            let pbPointsClamped = max(0, pbPoints)
            message += " Your PB: \(pbFormatted). That's \(Int(pbPointsClamped)) points."
        }
    ...
    /// Announce tetrathlon checkpoint with PB comparison and projected points
    func announceTetrathlonCheckpoint(
        distanceCovered: Double,
        totalDistance: Double,
        currentTime: TimeInterval,
        expectedPBTime: TimeInterval,
        standardTime: TimeInterval
    ) {
        guard isEnabled else { return }
        ...
        if distanceCovered >= 1000 {
            ...
            distanceText = "\(Int(distanceCovered)) metres"
        }
        ...
        // PB comparison
        if expectedPBTime > 0 {
            let timeDiff = currentTime - expectedPBTime
            let absDiff = Int(abs(timeDiff))
            if abs(timeDiff) <= 2 {
                message += "On PB pace. "
            } else if timeDiff < 0 {
                message += "\(absDiff) seconds ahead of PB. "
            } else {
                message += "\(absDiff) seconds behind PB. "
            }
        }

        // Projected points
        let percentComplete = distanceCovered / totalDistance
        if percentComplete > 0.1 {
            let projectedTime = currentTime / percentComplete
            let projectedPoints = max(0, 1000.0 - ((projectedTime - standardTime) * 3.0))
            message += "On track for \(Int(projectedPoints)) points."

            if projectedPoints >= 1000 {
                message += " Brilliant!"
            } else if projectedPoints >= 900 {
                message += " Strong!"
            }
        }
    ...
    /// Announce tetrathlon race completion with final points
    func announceTetrathlonComplete(
        ...
        let timeFormatted = formatDurationForSpeech(finalTime)
        let points = max(0, 1000.0 - ((finalTime - standardTime) * 3.0))

        var message = "Finished in \(timeFormatted). \(Int(points)) tetrathlon points."

        if isNewPB {
            let improvement = Int(abs(finalTime - pbTime))
            message += " Congratulations! New personal best by \(improvement) seconds!"
        } else if pbTime > 0 {
            let diff = Int(finalTime - pbTime)
            if diff <= 3 {
                message += " Matched your PB. Great consistency!"
            } else {
                message += " \(diff) seconds off your best. Good effort!"
            }
        }
...
extension AudioCoachManager {
    ...
    /// Process ground contact time and announce if too high
    /// - Parameter gct: Ground contact time in milliseconds
    func processGroundContactTime(_ gct: Double) {
        guard isEnabled, announceRunningBiomechanics else { return }
        ...
        if gct > 300 {
            ...
            announce("Think hot coals, spend less time on the ground")
        } else if gct > 280 {
            Self.lastGroundContactWarning = Date()
            announce("Focus on quick, light steps")
        }
    }

    private static var lastOscillationWarning: Date?

    /// Process vertical oscillation and announce if too high
    /// - Parameter oscillation: Vertical oscillation in centimeters
    func processVerticalOscillation(_ oscillation: Double) {
        guard isEnabled, announceRunningBiomechanics else { return }

        if let lastWarning = Self.lastOscillationWarning,
           Date().timeIntervalSince(lastWarning) < 60 {
            return
        }

        if oscillation > 12 {
            Self.lastOscillationWarning = Date()
            announce("Run tall, imagine a string pulling you up")
        } else if oscillation > 10 {
            Self.lastOscillationWarning = Date()
            announce("Engage your core and stay low")
        }
    }
...
extension AudioCoachManager {
    ...
    /// Announce walking session start
    func announceWalkingSessionStart(routeName: String?) {
        guard isEnabled else { return }
        if let name = routeName {
            announce("Walking session started. Route: \(name). Enjoy your walk.")
        } else {
            announce("Walking session started. Enjoy your walk.")
        }
    }

    /// Announce walking session end with summary
    func announceWalkingSessionEnd(distance: Double, duration: TimeInterval, averageCadence: Int) {
        guard isEnabled else { return }
        let distStr = formatDistanceForSpeech(distance)
        let durStr = formatTimeForSpeech(duration)
        var message = "Walk complete. \(distStr) in \(durStr)."
        if averageCadence > 0 {
            message += " Average cadence \(averageCadence) steps per minute."
        }
        announce(message)
    }
    ...
    func announceWalkingCadenceFeedback(currentCadence: Int, targetCadence: Int) {
        ...
        if abs(deviation) <= 5 {
            ...
            announce("Cadence \(currentCadence). Try to ease back towards \(targetCadence) steps per minute.")
        } else {
            announce("Cadence \(currentCadence). Try to pick up the pace slightly towards \(targetCadence) steps per minute.")
        }
    }
    ...
    /// Announce route comparison when walking a known route
    func announceWalkingRouteComparison(routeName: String, paceChange: Double, isImproved: Bool) {
        guard isEnabled else { return }
        let changeStr = String(format: "%.0f seconds per kilometre", abs(paceChange))
        if isImproved {
            announce("On your \(routeName) route. \(changeStr) faster than last time. Nice progress!")
        } else {
            announce("On your \(routeName) route. \(changeStr) slower than last time. Keep it steady.")
        }
    }
}
