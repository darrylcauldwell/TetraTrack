...
struct TetraTrackApp: App {
    ...
    private func handleScenePhaseChange(from oldPhase: ScenePhase, to newPhase: ScenePhase) {
        let hasActiveSession = sessionTracker?.sessionState == .tracking || sessionTracker?.sessionState == .paused
    ...
    private func handleStartRide(notification: Notification) {
        ...
        if let rideTypeRaw = notification.userInfo?["rideType"] as? String,
           let parsed = RideType(rawValue: rideTypeRaw) {
            rideType = parsed
        }

        Task {
            let plugin = RidingPlugin(rideType: rideType)
            await tracker.start(plugin: plugin)
        }
    }
