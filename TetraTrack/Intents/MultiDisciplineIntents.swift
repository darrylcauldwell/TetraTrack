...
final class MultiDisciplineDataManager {
    ...
    func fetchAllDisciplineStats(for period: StatsPeriodEnum) throws -> [DisciplineEnum: DisciplineStats] {
        ...
        let context = container.mainContext
        let (rides, runs, swims, shoots) = SessionQueryService.fetchByDiscipline(
            context: context,
            from: period.dateRange.start
        )

        var results: [DisciplineEnum: DisciplineStats] = [:]
        results[.riding] = DisciplineStats(
            sessions: rides.count,
            duration: rides.reduce(0) { $0 + $1.totalDuration }
        )
        results[.running] = DisciplineStats(
            sessions: runs.count,
            duration: runs.reduce(0) { $0 + $1.totalDuration }
        )
        results[.swimming] = DisciplineStats(
            sessions: swims.count,
            duration: swims.reduce(0) { $0 + $1.totalDuration }
        )
        results[.shooting] = DisciplineStats(
            sessions: shoots.count,
            duration: shoots.reduce(0) { $0 + $1.totalDuration }
        )
        return results
    }
}
...
// Note: All App Shortcuts are consolidated in TetraTrackShortcuts in RideIntents.swift
