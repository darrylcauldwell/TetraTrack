...
enum RelationshipType: String, Codable, CaseIterable {
    ...
    var color: Color {
        ...
        case .familyMember: return .green
        }
    }
}
...
final class SharingRelationship {
    ...
    /// Activity-agnostic alias for live tracking permission.
    /// Reads/writes the existing `canViewLiveRiding` field (CloudKit-safe).
    var canViewLiveTracking: Bool {
        get { canViewLiveRiding }
        set { canViewLiveRiding = newValue }
    }
    ...
    var enabledFeatureCount: Int {
        [canViewLiveTracking, canViewTrainingSummaries, canViewCompetitions,
    ...
    func shouldReceiveAlert(for discipline: TrainingDiscipline, alertType: AlertType) -> Bool {
        ...
        case .liveTracking:
            return canViewLiveTracking
        case .competitionReminder:
            return receiveCompetitionReminders && canViewCompetitions
        case .safety:
            return canViewLiveTracking  // Safety alerts always sent if live tracking enabled
        }
    }
