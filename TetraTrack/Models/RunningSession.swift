...
final class RunningSession: TrainingSessionProtocol, PaceBasedSessionProtocol, ElevationSessionProtocol, HeartRateSessionProtocol, CadenceSessionProtocol {
    ...
    var targetCadence: Int = 0 // 0 means no target set
    ...
    var healthKitHRRecoveryOneMinute: Double?   // bpm drop in 60s post-exercise

    // Walking-specific HealthKit metrics (Apple Watch)
    var healthKitDoubleSupportPercentage: Double?   // percentage (lower = better)
    var healthKitWalkingSpeed: Double?              // m/s
    var healthKitWalkingStepLength: Double?         // meters
    var healthKitWalkingSteadiness: Double?         // 0-100 percentage
    var healthKitWalkingHeartRateAvg: Double?       // bpm (background metric)
    ...
    var trackLength: Double = 400.0  // track lap length in meters (configurable)
    ...
    // Coaching insights
    var coachingSummaryData: Data?  // Encoded RunningCoachingSummary

    // Segment PB analysis (hidden PBs found in longer runs)
    var segmentPBData: Data?  // Encoded [SegmentPBResult]

    // Walking biomechanics (populated for .walking session type)
    var walkingSymmetryScore: Double = 0    // 0-100, from HealthKit asymmetry
    var walkingRhythmScore: Double = 0      // 0-100, from cadence consistency
    var walkingStabilityScore: Double = 0   // 0-100, from speed variability
    var walkingCadenceConsistency: Double = 0 // coefficient of variation
    var matchedRouteId: UUID?               // links to WalkingRoute
    var routeComparisonData: Data?          // encoded WalkingRouteComparison

    // Training program link
    var programSessionId: UUID?             // links to ProgramSession
    ...
    var coachingSummary: RunningCoachingSummary? {
        get {
            guard let data = coachingSummaryData else { return nil }
            return try? JSONDecoder().decode(RunningCoachingSummary.self, from: data)
        }
        set {
            coachingSummaryData = try? JSONEncoder().encode(newValue)
        }
    }
    ...
    var segmentPBResults: [SegmentPBResult] {
        get {
            guard let data = segmentPBData else { return [] }
            return (try? JSONDecoder().decode([SegmentPBResult].self, from: data)) ?? []
        }
        set {
            segmentPBData = try? JSONEncoder().encode(newValue)
        }
    }

    var hasSegmentPBs: Bool {
        !segmentPBResults.isEmpty
    }

    /// Whether this session has coaching insights worth displaying
    var hasCoachingInsights: Bool {
        guard let summary = coachingSummary else { return false }
        return !summary.pbCheckpoints.isEmpty || !summary.pacerGapSnapshots.isEmpty || !summary.intervalPerformance.isEmpty
    }
    ...
    /// Whether this is a walking session
    var isWalking: Bool {
        sessionType == .walking
    }
    ...
    var routeComparison: WalkingRouteComparison? {
        get {
            guard let data = routeComparisonData else { return nil }
            return try? JSONDecoder().decode(WalkingRouteComparison.self, from: data)
        }
        set {
            routeComparisonData = try? JSONEncoder().encode(newValue)
        }
    }

    /// Whether this walking session has biomechanics scores
    var hasWalkingScores: Bool {
        walkingSymmetryScore > 0 || walkingRhythmScore > 0 || walkingStabilityScore > 0
    }
    ...
    /// Phase breakdown computed from GPS speed data
    ...
    /// Whether this session has Watch metrics data
    var hasWatchMetricsData: Bool {
        healthKitAsymmetry != nil || healthKitStrideLength != nil || healthKitPower != nil ||
        healthKitStepCount != nil || healthKitHRRecoveryOneMinute != nil ||
        healthKitDoubleSupportPercentage != nil || healthKitWalkingSteadiness != nil
    }
...
nonisolated struct RunningCoachingSummary: Codable, Sendable {
    ...
    var pacerGapSnapshots: [PacerGapSnapshot] = []
...
nonisolated struct PBCheckpointRecord: Codable, Identifiable, Sendable {
    ...
    var isAhead: Bool { currentTime < expectedTime }
