...
final class Competition {
    ...
    // Competition Day split times
    var swimmingSplitTimesData: Data?
    var runningSplitTimesData: Data?
    ...
    @Transient var swimmingSplitTimes: [TimeInterval] {
        get {
            guard let data = swimmingSplitTimesData else { return [] }
            return (try? JSONDecoder().decode([TimeInterval].self, from: data)) ?? []
        }
        set {
            swimmingSplitTimesData = try? JSONEncoder().encode(newValue)
        }
    }

    @Transient var runningSplitTimes: [TimeInterval] {
        get {
            guard let data = runningSplitTimesData else { return [] }
            return (try? JSONDecoder().decode([TimeInterval].self, from: data)) ?? []
        }
        set {
            runningSplitTimesData = try? JSONEncoder().encode(newValue)
        }
    }
