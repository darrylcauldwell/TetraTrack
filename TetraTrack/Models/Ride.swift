...
final class Ride: GaitTimeTracking, TrainingSessionProtocol {
    ...
    /// Number of jumps detected during session
    var detectedJumpCount: Int = 0

    /// Percentage of session time in active movement (0-100)
    var activeTimePercent: Double = 0.0
    ...
    /// Typed accessor for phone mount position
    var phoneMountPosition: PhoneMountPosition {
        get { PhoneMountPosition(rawValue: phoneMountPositionValue) ?? .jodhpurThigh }
        set { phoneMountPositionValue = newValue.rawValue }
    }
    ...
    var turnBalancePercent: Int {
        let totalAngle = totalLeftAngle + totalRightAngle
        guard totalAngle > 0 else { return 50 }
        return Int((totalLeftAngle / totalAngle) * 100)
    }
    ...
    // MARK: - Gait Diagnostics

    /// Decoded gait diagnostic entries (cached to avoid repeated JSON decoding)
    var gaitDiagnostics: [GaitDiagnosticEntry] {
        get {
            if let cached = _cachedGaitDiagnostics { return cached }
            guard let data = gaitDiagnosticsData else { return [] }
            do {
                let decoded = try JSONDecoder().decode([GaitDiagnosticEntry].self, from: data)
                _cachedGaitDiagnostics = decoded
                return decoded
            } catch {
                Log.app.error("Failed to decode gaitDiagnostics: \(error)")
                _cachedGaitDiagnostics = []
                return []
            }
        }
        set {
            gaitDiagnosticsData = try? JSONEncoder().encode(newValue)
            _cachedGaitDiagnostics = newValue
        }
    }
