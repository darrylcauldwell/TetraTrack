...
enum PBCategory: String, CaseIterable {
...
struct RunningPBEditorView: View {
    ...
    @State private var selectedCategory: PBCategory = .practice
    ...
    private var currentPBForCategory: TimeInterval {
        switch selectedCategory {
        case .practice: return personalBests.personalBest(for: distance)
        case .competition: return personalBests.competitionPersonalBest(for: distance)
        }
    }
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 24) {
                ...
                // Practice / Competition picker
                Picker("Category", selection: $selectedCategory) {
                    ForEach(PBCategory.allCases, id: \.self) { cat in
                ...
                // Show other category PB for reference
                let otherCategory: PBCategory = selectedCategory == .practice ? .competition : .practice
                let otherPB = otherCategory == .practice
                    ? personalBests.personalBest(for: distance)
                    : personalBests.competitionPersonalBest(for: distance)
                if otherPB > 0 {
                    let otherMins = Int(otherPB) / 60
                    let otherSecs = Int(otherPB) % 60
                    Text("\(otherCategory.rawValue) PB: \(otherMins):\(String(format: "%02d", otherSecs))")
                        .font(.caption)
                        .foregroundStyle(.tertiary)
                }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        ...
                        case .competition:
                            pbs.updateCompetitionPersonalBest(for: distance, time: totalSeconds)
                        }
                        dismiss()
                    }
    ...
    private func loadPBForCategory() {
        let currentPB = currentPBForCategory
        if currentPB > 0 {
