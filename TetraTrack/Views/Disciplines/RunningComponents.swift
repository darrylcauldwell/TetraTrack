...
struct RunningSessionDetailView: View {
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    // Phone Motion Analysis (when phone sensors were active)
                    if session.hasPhoneMotionData {
                        phoneMotionDetailSection
                    }

                    // Runner Insights link (when we have biomechanics data)
                    if session.averageCadence > 0 || session.averageVerticalOscillation > 0 || session.averageHeartRate > 0 || session.hasPhoneMotionData {
                        NavigationLink(destination: RunningInsightsView(session: session)) {
                            HStack {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    Text("GRACE Insights")
                                        .font(.headline)
                                    Text("Grow \u{00B7} Rhythm \u{00B7} Align \u{00B7} Circle \u{00B7} Enjoy")
                                        .font(.caption)
                                        .foregroundStyle(.secondary)
                                }
                                Spacer()
    ...
    // MARK: - Phone Motion Detail Section

    private var phoneMotionDetailSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Image(systemName: "iphone.gen3")
                    .foregroundStyle(AppColors.primary)
                Text("Phone Sensor Analysis")
                    .font(.headline)
            }

            // Metrics rows
            VStack(spacing: 0) {
                if session.phoneAverageCadence > 0 {
                    phoneMetricRow(icon: "metronome", label: "Cadence", value: "\(session.phoneAverageCadence) spm")
                }
                // HealthKit metrics from Apple Watch (reliable) - shown with watch icon
                if let healthKitAsymmetry = session.healthKitAsymmetry, healthKitAsymmetry > 0 {
                    Divider()
                    phoneMetricRow(icon: "applewatch", label: "Asymmetry", value: String(format: "%.1f%%", healthKitAsymmetry))
                }
                if let healthKitStrideLength = session.healthKitStrideLength, healthKitStrideLength > 0 {
                    Divider()
                    phoneMetricRow(icon: "applewatch", label: "Stride Length", value: String(format: "%.2f m", healthKitStrideLength))
                }
                if let healthKitPower = session.healthKitPower, healthKitPower > 0 {
                    Divider()
                    phoneMetricRow(icon: "applewatch", label: "Power", value: String(format: "%.0f W", healthKitPower))
                }
                if let healthKitSteps = session.healthKitStepCount, healthKitSteps > 0 {
                    Divider()
                    phoneMetricRow(icon: "applewatch", label: "Steps", value: "\(healthKitSteps)")
                }
                if session.averageImpactLoadValue > 0 {
                    Divider()
                    phoneMetricRow(icon: "waveform.path.ecg", label: "Impact", value: String(format: "%.1fg", session.averageImpactLoadValue))
                }
            }

            // Impact load trend
            if session.impactLoadTrendValue != 0 {
                HStack {
                    Text("Impact Trend")
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
                    Spacer()
                    HStack(spacing: 2) {
                        Image(systemName: session.impactLoadTrendValue > 0 ? "arrow.up.right" : "arrow.down.right")
                            .foregroundStyle(session.impactLoadTrendValue > 15 ? .red : .green)
                        Text(String(format: "%+.0f%%", session.impactLoadTrendValue))
                            .font(.subheadline)
                            .foregroundStyle(session.impactLoadTrendValue > 15 ? .red : .green)
                    }
                }
            ...
            // Step count
            if session.totalStepCount > 0 {
                HStack {
                    Image(systemName: "shoeprints.fill")
                        .foregroundStyle(.secondary)
                    Text("\(session.totalStepCount) steps")
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    private func phoneMetricRow(icon: String, label: String, value: String) -> some View {
        HStack {
            Image(systemName: icon)
                .font(.subheadline)
                .foregroundStyle(AppColors.primary)
