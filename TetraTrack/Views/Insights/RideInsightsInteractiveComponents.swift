...
struct ExpandedMetricDetailView: View {
    ...
    private func effortVisualization(in size: CGSize) -> some View {
        Path { path in
            let points = segments.enumerated().compactMap { index, segment -> CGPoint? in
                ...
                let y = size.height * (1 - CGFloat(segment.rhythmScore / 100))
...
extension InsightSection {
    var detailTitle: String {
        ...
        case .effort: return "Effort level over time"
        }
    }
    ...
    var color: Color {
        ...
        case .effort: return AppColors.error
        }
    }
}
