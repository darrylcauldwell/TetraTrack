...
struct TrainingLoadDashboardView: View {
                ...
                Chart {
                    ForEach(weeklyData, id: \.week) { week in
                        ...
                        if week.walking > 0 {
                            BarMark(
                                x: .value("Week", week.week),
                                y: .value("TSS", week.walking)
                            )
                            .foregroundStyle(by: .value("Discipline", "Walking"))
                        }
                        if week.swimming > 0 {
                            BarMark(
                                x: .value("Week", week.week),
    ...
    // MARK: - Training Load Explainer

    private var trainingLoadExplainer: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text("Tip: Aim to grow CTL gradually over time while managing ATL with planned rest. Taper before competitions by reducing training volume to let TSB rise into the positive zone.")
                .font(.caption)
                .foregroundStyle(.secondary)
                .padding(.top, 4)
        }
    ...
    private func explainerRow(color: Color, abbreviation: String, title: String, description: String) -> some View {
