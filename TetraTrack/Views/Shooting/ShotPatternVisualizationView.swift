...
struct ShotPatternVisualizationView: View {
        ...
        HStack(spacing: 0) {
            ForEach(ShotDisplayMode.allCases, id: \.self) { mode in
                Button {
                    ...
                    .background(displayMode == mode ? AppColors.primary : Color.clear)
    ...
    private func ringColor(for score: Int) -> Color {
        switch score {
        case 10: return AppColors.scoringRing10
        case 8: return AppColors.scoringRing8
        case 6: return AppColors.scoringRing6
    ...
    private func mpiOverlay(size: CGSize) -> some View {
        ...
        return ZStack {
            ...
            if let data = visualData, let mpiCurrent = data.mpiCurrent {
                ...
                MPIMarker(color: AppColors.shotCurrent, label: "MPI")
                    .position(mpiScreen)
            }
            ...
            if showAggregate, let data = visualData, let mpiAggregate = data.mpiAggregate {
                ...
                MPIMarker(color: AppColors.shootingCompetition, label: "Avg")
                    .position(mpiScreen)
            }
        }
    }
    ...
    private func groupRadiusOverlay(size: CGSize) -> some View {
        ...
        return ZStack {
            ...
            if let data = visualData, let mpiCurrent = data.mpiCurrent, let radiusCurrent = data.groupRadiusCurrent {
                ...
                Circle()
                    .stroke(AppColors.shotCurrent.opacity(0.5), style: StrokeStyle(lineWidth: 2, dash: [5, 3]))
            ...
            if showAggregate, let data = visualData, let mpiAggregate = data.mpiAggregate, let radiusAggregate = data.groupRadiusAggregate {
                ...
                Circle()
                    .stroke(AppColors.shootingCompetition.opacity(0.5), style: StrokeStyle(lineWidth: 2, dash: [5, 3]))
                    .frame(width: radiusScreen * 2, height: radiusScreen * 2)
                    .position(mpiScreen)
            }
        }
    ...
    private func ringLabelColor(for score: Int) -> Color {
        ...
        case 8: return AppColors.scoringRing8
        case 6: return AppColors.scoringRing6
        case 4: return AppColors.inactive
        case 2: return .secondary
    ...
    private var controlsRow: some View {
        HStack(spacing: 16) {
            ...
            .toggleStyle(.button)
            .tint(AppColors.shootingCompetition)
                ...
                HStack(spacing: 6) {
                    ...
                    if shot.isOutlier {
                        Text("Outlier")
                            ...
                            .padding(.vertical, 2)
                            .background(AppColors.shotOutlier)
                            .clipShape(Capsule())
                    }
                }
...
struct ShotMarker: View {
    ...
    var body: some View {
        ZStack {
            ...
            if isSelected {
                Circle()
                    .stroke(AppColors.shotSelected.opacity(0.5), lineWidth: 2)
    ...
    private var fillColor: Color {
        if isOutlier {
            return AppColors.shotOutlier.opacity(0.6)
        } else if isHistorical {
            return AppColors.shotHistorical.opacity(0.4)
        } else {
            return AppColors.shotCurrent.opacity(0.7)
        }
    }

    private var strokeColor: Color {
        if isSelected {
            return AppColors.shotSelected
        } else if isOutlier {
            return AppColors.shotOutlier
        } else if isHistorical {
            return AppColors.shotHistorical
        } else {
            return AppColors.shotCurrent
        }
    }
