...
struct ShootingInsightsView: View {
    let onDismiss: () -> Void
    var shootingSessions: [ShootingSession] = []
    ...
    private var contentView: some View {
        ScrollView {
            VStack(spacing: 20) {
                ...
                // Sensor-based pressure comparison (when Watch data available)
                if !sensorSessionsByContext.isEmpty {
                    sensorPressureSection
                }
            }
            .padding(.vertical)
        }
    ...
    private var sensorSessionsByContext: [ShootingSessionContext: [ShootingSession]] {
        Dictionary(grouping: sessionsWithSensorData, by: { $0.sessionContext })
    }

    // MARK: - Sensor Pressure Section

    private var sensorPressureSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Image(systemName: "applewatch.radiowaves.left.and.right")
                    .foregroundStyle(.cyan)
                Text("Watch Sensor Comparison")
                    .font(.headline)
            }
            .padding(.horizontal)

            // Average steadiness by context
            VStack(alignment: .leading, spacing: 8) {
                Text("Steadiness by Context")
                    .font(.subheadline.weight(.medium))
                    .padding(.horizontal)

                ForEach(ShootingSessionContext.allCases, id: \.self) { context in
                    if let sessions = sensorSessionsByContext[context], !sessions.isEmpty {
                        let avgSteadiness = sessions.map(\.averageHoldSteadiness).reduce(0, +) / Double(sessions.count)
                        HStack {
            ...
            // Average HR by context
            let contextHR = sensorSessionsByContext.compactMapValues { sessions -> Int? in
                let hrs = sessions.map(\.averageHeartRate).filter { $0 > 0 }
                guard !hrs.isEmpty else { return nil }
                return hrs.reduce(0, +) / hrs.count
            }
            if !contextHR.isEmpty {
                Divider()
                    .padding(.vertical, 4)

                VStack(alignment: .leading, spacing: 8) {
                    Text("Heart Rate by Context")
                        .font(.subheadline.weight(.medium))
                        .padding(.horizontal)

                    ForEach(ShootingSessionContext.allCases, id: \.self) { context in
                        if let hr = contextHR[context] {
                            HStack {
                    ...
                    // Nerves indicator
                    if let practiceHR = contextHR[.freePractice], let compHR = contextHR[.competition] {
                        let diff = compHR - practiceHR
                        if diff > 5 {
                            HStack(alignment: .top, spacing: 6) {
                                Image(systemName: "exclamationmark.triangle.fill")
                                    .font(.caption)
                                    .foregroundStyle(.orange)
                                Text("Heart rate rises +\(diff) bpm in competitions vs practice — consider pre-competition breathing routines")
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
                            }
                            .padding(.horizontal)
            ...
            // Composure score trend
            if sessionsWithSensorData.count >= 3 {
                Divider()
                    .padding(.vertical, 4)

                VStack(alignment: .leading, spacing: 8) {
                    Text("Composure Trend")
                        .font(.subheadline.weight(.medium))
                        .padding(.horizontal)

                    let sorted = sessionsWithSensorData.sorted { $0.startDate < $1.startDate }
                    Chart {
    ...
    private func steadinessBar(_ value: Double) -> some View {
