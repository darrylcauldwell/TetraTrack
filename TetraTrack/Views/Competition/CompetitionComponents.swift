...
struct CountdownUnit: View {
    ...
    var body: some View {
        VStack(spacing: 4) {
            Text("\(value)")
                .scaledFont(size: 28, weight: .bold, design: .monospaced, relativeTo: .title2)
...
struct CompetitionDetailView: View {
    ...
    @State private var calendarSyncMessage = ""
        ...
        NavigationStack {
                        ...
                        if discoveredPhotos.isEmpty && discoveredVideos.isEmpty && competition.photos.isEmpty && competition.videoAssetIdentifiers.isEmpty {
                            ...
                            ScrollView(.horizontal, showsIndicators: false) {
                                HStack(spacing: 8) {
                                    ...
                                    ForEach(Array(competition.photos.prefix(2).enumerated()), id: \.element) { _, photoData in
            ...
            .confirmationDialog("Delete Competition", isPresented: $showingDeleteConfirmation) {
                Button("Delete", role: .destructive) {
                    ...
                    Task {
                        await CalendarSyncService.shared.removeCompetition(competition)
                    }
            ...
            .alert("Calendar Sync", isPresented: $showingCalendarSyncAlert) {
                Button("OK") { }
            } message: {
...
struct CompetitionEditView: View {
    ...
    private func save() {
        ...
        // Update calendar event if already synced
        if !comp.calendarEventIdentifier.isEmpty {
            Task {
                await CalendarSyncService.shared.syncCompetition(comp)
            }
        }
    ...
    private func calculateRouteTime() {
        ...
        Task {
            ...
            #if compiler(>=6.2)
            request.destination = MKMapItem(location: CLLocation(latitude: destCoord.latitude, longitude: destCoord.longitude), address: nil)
            #else
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                if !competition.photos.isEmpty {
                    ...
                    LazyVGrid(columns: columns, spacing: 8) {
                        ForEach(Array(competition.photos.enumerated()), id: \.element) { _, photoData in
                            if let uiImage = UIImage(data: photoData) {
                                Image(uiImage: uiImage)
                                    .resizable()
...
struct CalendarSyncButton: View {
    ...
    private var calendarService: CalendarSyncService {
        CalendarSyncService.shared
    }

    private var isSynced: Bool {
        !competition.calendarEventIdentifier.isEmpty
    }
        ...
        VStack(spacing: 12) {
            HStack {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("Apple Calendar")
                        .font(.headline)
                    Text(isSynced ? "Synced to TetraTrack calendar" : "Add to your Apple Calendar")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            ...
            HStack(spacing: 12) {
                if isSynced {
                    Button {
                        Task { await updateCalendarEvent() }
                    } label: {
                    ...
                    Button(role: .destructive) {
                        Task { await removeCalendarEvent() }
                    } label: {
    ...
    private func syncToCalendar() async {
        ...
        let success = await calendarService.syncCompetition(competition)
        if success {
            alertMessage = "Competition added to your TetraTrack calendar."
            showingAlert = true
        } else {
            alertMessage = calendarService.lastError ?? "Failed to sync to calendar."
            showingAlert = true
        }
    }

    private func updateCalendarEvent() async {
        isInProgress = true
        defer { isInProgress = false }

        let success = await calendarService.syncCompetition(competition)
        if success {
            alertMessage = "Calendar event updated."
            showingAlert = true
        } else {
            alertMessage = calendarService.lastError ?? "Failed to update calendar event."
            showingAlert = true
        }
    }
    ...
    private func removeCalendarEvent() async {
        ...
        let success = await calendarService.removeCompetition(competition)
        if success {
            alertMessage = "Competition removed from calendar."
            showingAlert = true
        } else {
            alertMessage = calendarService.lastError ?? "Failed to remove calendar event."
            showingAlert = true
        }
    }
}
