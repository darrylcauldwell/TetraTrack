...
struct CompetitionTrendChart: View {
    ...
    private var trendChartSummary: String {
        let pointsWithData = trendPoints.filter { $0.totalPoints != nil }
        guard !pointsWithData.isEmpty else { return "No competition data available" }
        let minPoints = pointsWithData.compactMap { $0.totalPoints }.min() ?? 0
        let maxPoints = pointsWithData.compactMap { $0.totalPoints }.max() ?? 0
        return "Competition points trend showing \(pointsWithData.count) competitions with scores ranging from \(Int(minPoints)) to \(Int(maxPoints)) points"
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            .frame(height: 200)
            .accessibleChart(trendChartSummary)
        }
...
struct DisciplineBreakdownChart: View {
    ...
    private var breakdownChartSummary: String {
        guard !disciplineData.isEmpty else { return "No discipline data available" }
        let summary = disciplineData.map { "\($0.name) \(String(format: "%.0f", $0.points)) points" }
        return "Discipline breakdown chart: \(summary.joined(separator: ", "))"
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if disciplineData.isEmpty {
                ...
                .frame(height: 200)
                .accessibleChart(breakdownChartSummary)
                ...
                HStack(spacing: 16) {
                    ForEach(disciplineData, id: \.name) { item in
                        ...
                        .accessibilityLabel("\(item.name): \(String(format: "%.0f", item.points)) average points")
                    }
                }
                .frame(maxWidth: .infinity)
            }
