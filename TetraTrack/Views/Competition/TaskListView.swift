...
struct TaskListView: View {
    ...
    private var remindersService: RemindersSyncService { RemindersSyncService.shared }
            ...
            .toolbar {
                ToolbarItem(placement: .topBarLeading) {
                    Button(action: syncToReminders) {
                        Label("Sync to Reminders", systemImage: remindersService.syncStatus == .syncing
                              ? "arrow.triangle.2.circlepath"
                              : "arrow.triangle.2.circlepath.circle")
                    }
                    .disabled(remindersService.syncStatus == .syncing)
                }
    ...
    @ViewBuilder
    private var remindersSyncStatusBanner: some View {
        if remindersService.syncStatus != .idle {
            HStack(spacing: 8) {
                Image(systemName: remindersService.syncStatus.icon)
                    .font(.caption)
                    .foregroundStyle(syncStatusColor)
                    .symbolEffect(.pulse, isActive: remindersService.syncStatus == .syncing)

                Text(remindersService.syncStatus.displayText)
                    .font(.caption)
                    .foregroundStyle(syncStatusColor)

                Spacer()

                if case .error = remindersService.syncStatus {
                    Button("Retry") {
                        syncToReminders()
                    }
                    .font(.caption)
    ...
    private var syncStatusColor: Color {
        ...
        case .synced: return AppColors.active
    ...
    private func syncToReminders() {
        Task {
            var hasAccess = remindersService.hasAccess
            if !hasAccess {
                hasAccess = await remindersService.requestAccess()
            }
            guard hasAccess else {
                remindersService.syncStatus = .error("Access denied")
                return
            }
            remindersService.syncAllTasks(allTasks)
        }
    }

    private func syncSingleTaskToReminders(_ task: CompetitionTask) {
        guard remindersService.hasAccess else { return }
        remindersService.syncTask(task)
    }

    private func fetchRemindersCompletionUpdates() {
        guard remindersService.hasAccess else { return }
        remindersService.fetchCompletionUpdates(allTasks)
    }
    ...
    private func toggleTask(_ task: CompetitionTask) {
        ...
        // Sync to Reminders
        syncSingleTaskToReminders(task)
    ...
    private func deleteTask(_ task: CompetitionTask) {
        // Remove from Reminders before deleting
        RemindersSyncService.shared.removeTask(task)
...
struct TaskEditView: View {
    ...
    private func save() {
        ...
        // Sync to Reminders if access was previously granted
        let remindersService = RemindersSyncService.shared
        if remindersService.hasAccess {
            remindersService.syncTask(taskToSave)
        }
                    ...
                    ForEach(pendingTasks) { task in
                        CompactTaskRow(task: task) {
                            ...
                            if RemindersSyncService.shared.hasAccess {
                                RemindersSyncService.shared.syncTask(task)
                            }
                        } onTap: {
                            selectedTask = task
                        }
