...
struct SharingDiagnosticsView: View {
        ...
        List {
            ...
            Section {
                HStack {
                    Text("Build Type")
                    Spacer()
                    Text(SceneDelegate.buildEnvironmentDescription)
                        .foregroundStyle(.secondary)
                }

                VStack(alignment: .leading, spacing: 4) {
                    Text("Both phones must show the same build type for sharing to work.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    Text("Xcode builds use Development CloudKit. TestFlight uses Production. Shares do not cross environments.")
                        .font(.caption)
                        .foregroundStyle(.orange)
                }
            } header: {
            ...
            Section {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    BulletPoint(text: "Both phones must use the same build type (both Xcode OR both TestFlight)")
    ...
    private func resetSharingData() async {
        ...
        // 2. Reset ALL in-memory state: service caches, linked riders,
        //    SwiftData records, rate limits, relationship connection info
        await UnifiedSharingCoordinator.shared.resetAllSharingState()

        // 3. Re-run setup to refresh account status and recreate zone
        await UnifiedSharingCoordinator.shared.setup()

        if errors.isEmpty {
            resetResultMessage = "Successfully reset all sharing data:\n• Deleted FamilySharing zone and all CloudKit records\n• Cleared service caches and linked riders\n• Recreated zone for fresh start\n\nYou can now re-send invite links."
        } else {
