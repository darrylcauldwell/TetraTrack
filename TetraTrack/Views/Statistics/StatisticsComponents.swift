...
struct WeeklyActivityChart: View {
    ...
    private var chartSummary: String {
        let totalRides = data.reduce(0) { $0 + $1.rideCount }
        let totalDistance = data.reduce(0.0) { $0 + $1.distanceKm }
        return "Weekly activity chart showing \(totalRides) rides totaling \(String(format: "%.1f", totalDistance)) kilometers over \(data.count) weeks"
    }
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack {
                ForEach(data) { week in
                    ...
                    .accessibilityLabel("\(week.formattedWeek): \(week.rideCount) rides, \(String(format: "%.1f", week.distanceKm)) kilometers")
                }
            }
        }
        .glassCard(material: .thin, cornerRadius: 20, padding: 20)
...
struct GaitAnalysisView: View {
    ...
    private var gaitChartSummary: String {
        let breakdown = statistics.gaitBreakdown.map { "\($0.gait.rawValue) \(String(format: "%.0f", $0.percentage))%" }
        return "Gait distribution chart: \(breakdown.joined(separator: ", "))"
    }
            ...
            if statistics.gaitBreakdown.isEmpty {
                ...
                HStack(spacing: 24) {
                    ZStack {
                        ...
                        .accessibleChart(gaitChartSummary)
                    }
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ForEach(statistics.gaitBreakdown, id: \.gait) { item in
                            ...
                            .accessibilityElement(children: .combine)
                            .accessibilityLabel("\(item.gait.rawValue): \(String(format: "%.0f", item.percentage)) percent")
                        }
                    }
                }
