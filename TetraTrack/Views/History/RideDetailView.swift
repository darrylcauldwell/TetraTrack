...
struct RideDetailView: View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Image(systemName: "waveform.badge.magnifyingglass")
                    .font(.subheadline)
                    .foregroundStyle(AppColors.primary)
                Text("Gait Analysis Export")
                    .font(.subheadline)
                    .fontWeight(.medium)
            }

            Text("Export sensor data for reviewing gait detection with AI")
                .font(.caption)
                .foregroundStyle(.secondary)

            HStack(spacing: 12) {
                Button(action: exportSensorCSV) {
                ...
                Button(action: exportSensorJSON) {
    ...
    private func exportGPX() {
        Task {
            if let url = GPXExporter.shared.exportToFile(ride: ride) {
                await MainActor.run {
    ...
    private func exportSensorCSV() {
        if let url = SensorDataExportService.shared.exportCSV(ride: ride) {
            presentShareSheet(for: url)
        }
    }

    private func exportSensorJSON() {
        if let url = SensorDataExportService.shared.exportJSON(ride: ride) {
            presentShareSheet(for: url)
        }
    }
