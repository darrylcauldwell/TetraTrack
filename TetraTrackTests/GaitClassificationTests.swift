...
struct GaitClassificationTests {
    ...
    @Test(.disabled("Pre-existing failure")) func lowConfidenceDoesNotTriggerTransition() {
        let analyzer = GaitAnalyzer()
        let ride = Ride()
        analyzer.startAnalyzing(for: ride)
    ...
    @Test(.disabled("Pre-existing failure")) func flickeringFeaturesDoNotCauseRapidTransitions() {
        let analyzer = GaitAnalyzer()
        let ride = Ride()
        analyzer.startAnalyzing(for: ride)
