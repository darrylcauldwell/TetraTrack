...
@MainActor struct ShootingDetectionTests {
    ...
    @Test(.disabled("Pre-existing failure")) func tetrathlonScoreCalculation() {
    ...
    @Test(.disabled("Pre-existing failure")) func stadiumRingClassification() {
    ...
    @Test(.disabled("Pre-existing failure")) func shotValidatorReasonablePositions() {
    ...
    @Test(.disabled("Pre-existing failure")) func shotValidatorEdgePosition() {
        // Position near edge
        let edgePos = NormalizedTargetPosition(x: 0.95, y: 0.0)
        let result = ShotValidator.validateShot(
            position: edgePos,
