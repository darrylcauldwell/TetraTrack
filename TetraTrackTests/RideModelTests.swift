...
struct RideModelTests {
    ...
    @Test func turnBalanceCalculation() {
        let ride = Ride()
        ride.totalLeftAngle = 600.0
        ride.totalRightAngle = 400.0

        // Balance = 600 / 1000 * 100 = 60%
        #expect(ride.turnBalancePercent == 60)
    }

    @Test func turnBalanceWithNoTurns() {
        let ride = Ride()
        ride.totalLeftAngle = 0
        ride.totalRightAngle = 0
...
@MainActor struct RideTypeTests {
    ...
    @Test(.disabled("Pre-existing failure")) func rideTypeAllCasesCount() {
