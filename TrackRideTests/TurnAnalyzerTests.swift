...
struct TurnAnalyzerTests {
    ...
    @Test func initialization() {
        ...
        #expect(analyzer.leftTurns == 0)
        #expect(analyzer.rightTurns == 0)
    ...
    @Test func turnStatsReturnsCorrectValues() {
        ...
        #expect(stats.leftTurns == 0)
        #expect(stats.rightTurns == 0)
...
struct TurnOverSegmentationTests {
    ...
    @Test func syntheticArenaRideOverSegmentation() {
        ...
        let rawDetectedTurns = analyzer.leftTurns + analyzer.rightTurns
    ...
    @Test func singleContinuousTurnSegmentation() {
        ...
        RESULTS:
        ═══════════════════════════════════════════════════════════════════════
        Raw detected left turns:    \(analyzer.leftTurns)
        Raw detected right turns:   \(analyzer.rightTurns)
        Total raw events:           \(analyzer.leftTurns + analyzer.rightTurns)
        Physical turns detected:    \(report.physicalTurns)
        ═══════════════════════════════════════════════════════════════════════
        """)

        // This single 90° turn should be counted as 1 physical turn
        // But the raw detector may count multiple events if angle/sample >= 30°
        // With 18°/sample, it should NOT trigger (< 30° threshold)
        // But with noise, some samples might cross the threshold

        print("Expected: 1 physical turn")
        print("Raw events generated: \(analyzer.leftTurns + analyzer.rightTurns)")

        if analyzer.leftTurns + analyzer.rightTurns > 1 {
            print("WARNING: Single turn was over-segmented into multiple events!")
        }
    }
    ...
    @Test func turnCountingAtDifferentSampleRates() {
        ...
        for rate in sampleRates {
            ...
            Sample Rate: \(String(format: "%4.1f", rate)) Hz
              ...
              Raw events: \(String(format: "%3d", analyzer.leftTurns + analyzer.rightTurns))
...
struct TurnMetricsValidationTests {
    ...
    @Test func timeBasedVsEventBasedMetrics() {
        ...
        let leftTurnsAfterLeftPhase = analyzer.leftTurns
        ...
        print("""
        ╔══════════════════════════════════════════════════════════════════════╗
        ║              TIME-BASED vs EVENT-BASED METRICS                       ║
        ╠══════════════════════════════════════════════════════════════════════╣
        ║ EVENT-BASED (may be inflated):                                       ║
        ║   Left turn events:       \(String(format: "%10d", analyzer.leftTurns))                     ║
        ║   Right turn events:      \(String(format: "%10d", analyzer.rightTurns))                     ║
        ║   Event-based balance:    \(String(format: "%10.1f", Double(analyzer.leftTurns) / Double(max(1, analyzer.leftTurns + analyzer.rightTurns)) * 100))%%                    ║
        ╠══════════════════════════════════════════════════════════════════════╣
        ...
        // Event counts may vary due to threshold crossings
        print("\nNote: Event counts (\(analyzer.leftTurns + analyzer.rightTurns)) may differ from actual turns")
        print("Time-based balance (\(String(format: "%.1f", report.leftTurnPercent))%) is the reliable metric")
    }
